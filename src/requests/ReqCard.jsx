...
function ReqPhotos({ req }) {
  ...
  return (
    <div className="req-photos-grid">
      {photos.map((src, i) => <ReqPhoto key={src.slice(-20) + i} src={src} />)}
    </div>
  );
}
...
export function ReqCard({ req, userRole, userName, staggerIdx = 0, onRepeat, onEdit, onDelete, onCancel, highlightId, onHighlighted }) {
...
export function GroupedReqList({ reqs, userRole, userName, onRepeat, onEdit, onDelete, onCancel }) {
