...
import { useActions, useRequests, useAppDispatch } from '../store/AppStore.jsx';
...
import { setStatusWithHistory } from '../domain/requestWorkflow';
...
export default function ResidentView({ user, activeTab, setActiveTab }) {
  ...
  const onCancel = id => {
    updateRequest(id, { status: 'cancelled', cancelledAt: new Date() });
    setStatusWithHistory(dispatch, id, 'cancelled', 'Отменено жильцом', user.name, user.role);
  ...
  const filteredPasses = useMemo(() => {
    ...
    if (passFilter === 'active') return myPasses.filter(r => r.status !== 'cancelled' && r.status !== 'rejected' && r.status !== 'expired');
  ...
  return (
    <>
      ...
      {modal    && <CreateModal key={Date.now() + '_' + modal.cat} user={user} type={modal.type} initialCat={modal.cat} initialData={modal.data} onClose={() => setModal(null)} onDone={() => { setActiveTab(modal.type === 'tech' ? 'tech' : 'passes'); setModal(null); }} />}
