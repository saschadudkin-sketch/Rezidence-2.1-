 ...
 * Для перехода на live достаточно включить FEATURES.FIREBASE_LIVE.
 * и раскомментировать реальные реализации.
 */

// ─── Режим ───────────────────────────────────────────────────────────────────

import { MODE, isLiveMode } from '../config/runtimeMode';

/** Режим работы Firebase: 'demo' или 'live' (управляется через FEATURES.FIREBASE_LIVE) */
export const FIREBASE_MODE = MODE;
...
export const fetchPerms = async (uid) => {
  if (!isLiveMode()) return { visitors: [], workers: [] };
